  ...
  useEffect(() => {
    const fetchData = async () => {
      if (mode === EDIT) {
        const { id } = param;
        try {
          const document = await FirestoreService.getDocument('jenisClash', id);
  ...
  const onFinish = () => {
    ...
    form.validateFields().then(values => {
      setTimeout(() => {
        ...
        if (mode === ADD) {
          message.success(`Created ${values.namaJenisClash} to Jenis Clash`);
  ...
  const handleFinish = async (values) => {
    try {
      if (mode === ADD) {
        ...
        const { id } = param;
        await FirestoreService.updateDocument('jenisClash', id, values);
        message.success(`Jenis Clash updated successfully`);
      }
      form.resetFields();
      onFinish();
    } catch (error) {
